...
describe('getFetchClient', () => {
  ...
  it('should call correct url', async () => {
    ...
    expect(window.fetch).toHaveBeenCalledWith(
      'http://192.168.0.138:1337/test-fetch-client',
      expect.anything()
    );
  });
  ...
  it('should serialize a params object to a string', async () => {
    ...
    expect(data).toEqual({ data: 'success response' });
    expect(window.fetch).toHaveBeenCalledWith(
      'http://192.168.0.138:1337/test-fetch-client?page=1&pageSize=10&sort=short_text:ASC&filters[$and][0][biginteger][$eq]=3&filters[$and][1][short_text][$eq]=test&locale=en',
      expect.anything()
    );
  });
});
